import { Controller, Get, Post, Put, Delete, Body, Param, ParseUUIDPipe, HttpStatus, HttpCode, Req, UseGuards } from '@nestjs/common';
...
export class ArticleController {
    ...
    async show(@Param('id', new ParseUUIDPipe()) id: string): Promise<ReturnDto> {
        return <ReturnDto>{
            status: HttpStatus.OK,
            records: await this.articleService.findOne(
                { where: { id: id } }
            )
        }
    }
    ...
    @Get('/content/:id')
    async showByContentId(@Param('id', new ParseUUIDPipe()) id: string): Promise<ReturnDto> {
        return <ReturnDto>{
            status: HttpStatus.OK,
    ...
    @UseGuards(AuthGuard('jwt'))
    async update(@Param(
        'id', new ParseUUIDPipe()) id: string,
